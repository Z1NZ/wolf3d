...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   pfunc.type.d.lower.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: srabah <srabah@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/03/04 20:11:58 by srabah            #+#    #+#             */
/*   Updated: 2015/03/04 20:39:04 by srabah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		pfunc_type_d_lower_width_padding(int len, char **str)
{
	char	*tmp;
...
static void		pfunc_type_d_lower_precision(long long number, char **s_number)
{
	int		n_len;
	char	*tmp;
	char	*tmp1;
...
static void		pfunc_type_d_lower_width(long long number, char **str)
{
	int		len;
	char	*tmp;
	...
	if (WIDTH)
	{
		...
		(number < 0 || FLAG_MORE & BITCODE ||
			(FLAG_SPACE & BITCODE)) ? len-- : ' ';
		if (len > 0)
		{
			tmp = *str;
			pfunc_type_d_lower_width_padding(len, str);
			ft_memdel((void **)&tmp);
		}
	}
...
static void		pfunc_type_d_lower_casting(va_list *ap, long long *number)
{
...
int				pfunc_type_d_lower(va_list *ap)
{
	...
	number = 0;
	pfunc_type_d_lower_casting(ap, &number);
	s_number = pfunc_tool_itoa(ABS(number));
	pfunc_type_d_lower_precision(number, &s_number);
	pfunc_type_d_lower_width(number, &s_number);
	tmp = s_number;
